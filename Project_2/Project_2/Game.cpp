...
void Game::CheckCollisions() {
    ...
    if (FoxSprite.getGlobalBounds().intersects(CoinSprite.getGlobalBounds())) { //якщо спрайт торкаєть до спрайт то (використовується для перевірки перетину між двома геометричними обєктами)
        Money++;
